...
describe GreyGoo::Room do

	it "can serialize" do
		room = GreyGoo::Room.new({name: "test room", description: "test desc"})
		room.to_h.should include({name: "test room", description: "test desc"})
	end

	it "can add exits" do
		EM.synchrony do
			room1 = GreyGoo::Room.new({name: "room1"})
			room1.save!
			room2 = GreyGoo::Room.new({name: "room2"})
			room2.save!
			room1.add_exit("north", room2)
			room3 = GreyGoo::Room.load(room1._id)
	...
	it "can save and reload" do
		EM.synchrony do
			room = GreyGoo::Room.new({name: "test room 2", description: "test2"})
			room.save!
			id = room._id
			room2 = GreyGoo::Room.load(id)
	...
	it "can convert to resource" do
		EM.synchrony do
		room = GreyGoo::Room.new({name: "test room 3", description: "test desc"})
		room.save!
